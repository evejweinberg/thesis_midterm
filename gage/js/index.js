...
function drawScale() {
  ...
  var scaleB = document.querySelector(".scaleB");
  clearRect(scale)
  clearRect(scaleB)
  var n = 0;
  for (var sa = -180; sa <= 0; sa += 18) {
    ...
    var scaleLineB = document.createElementNS(NS, "line");
    ...
    setSVGAttributes(scaleLineB, scaleLineObjB);

    scale.appendChild(scaleLine);
    scaleB.appendChild(scaleLineB);

    var scaleText = document.createElementNS(NS, "text");
    var scaleTextB = document.createElementNS(NS, "text");
    ...
    // scaleText.textContent = n * 40;
    // scale.appendChild(scaleText);
    // scaleTextB.textContent = n * 10;
    // scaleB.appendChild(scaleTextB);
...
function drawInput(cx, cy, r1, offset, delta, a) {
  ...
  outlineB.setAttributeNS(null, "d", d1);
  fillB.setAttributeNS(null, "d", d2);
...
function drawNeedle(cx, cy, r1, a) {
  var randomNumMore = Math.floor(Math.random()*10)
  ...
  var pointsB = nx1+randomNumMore + "," + ny1+randomNumMore + " " + nx2+randomNumMore + "," + ny2+randomNumMore + " " + nx3+randomNumMore + "," + ny3+randomNumMore;

  var points = nx1 + "," + ny1 + " " + nx2 + "," + ny2 + " " + nx3 + "," + ny3;
  // needle.setAttributeNS(null, "points", points);

  // needleB.setAttributeNS(null, "points", pointsB);
}
